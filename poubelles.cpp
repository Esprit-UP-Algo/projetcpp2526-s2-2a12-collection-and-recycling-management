...
QSqlQueryModel* Poubelle::afficher() {
    ...
    QString queryStr = "SELECT ID_POUBELLE, TYPE, CAPACITE_MAX, NIVEAU_REMPLISSAGE, LOCALISATION, ETAT, DATE_VIDANGE, ID_ZONE FROM POUBELLES ORDER BY ID_POUBELLE";
    
    model->setQuery(queryStr, c->getDatabase());
    if (model->lastError().isValid() || model->rowCount() == 0) {
        queryStr = "SELECT ID_POUBELLE, TYPE, CAPACITE_MAX, NIVEAU_REMPLISSAGE, LOCALISATION, ETAT, DATE_VIDANGE, ID_ZONE FROM POUBELLE ORDER BY ID_POUBELLE";
        model->setQuery(queryStr, c->getDatabase());
    }
